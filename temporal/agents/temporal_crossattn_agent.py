"""
temporal_crossattn_agent.py — CARLA leaderboard agent for InterFuserTemporalCrossAttn (Approach 2).

Identical to temporal_agent.py but loads the cross-attention temporal model.
"""
...
DEMO_FRAMES_PATH = os.environ.get("DEMO_FRAMES_PATH", None)
IMAGENET_DEFAULT_MEAN = (0.485, 0.456, 0.406)
IMAGENET_DEFAULT_STD  = (0.229, 0.224, 0.225)


def get_entry_point():
    return "TemporalCrossAttnAgent"
...
class TemporalCrossAttnAgent(autonomous_agent.AutonomousAgent):
    ...
    def setup(self, path_to_conf_file):
        ...
        self.config = imp.load_source("TemporalCrossAttnConfig", path_to_conf_file).GlobalConfig()
        ...
        # Load cross-attention temporal model
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"Loading InterFuserTemporalCrossAttn from: {self.config.model_path}")
        ...
        self.net.eval()
        print(f"  CrossAttn model loaded. Params: {sum(p.numel() for p in self.net.parameters()):,}")
        ...
        self._demo_frames_path = None
        self._demo_frame_idx = 0
        if DEMO_FRAMES_PATH is not None:
            self._demo_frames_path = pathlib.Path(DEMO_FRAMES_PATH)
            self._demo_frames_path.mkdir(parents=True, exist_ok=True)
            print(f"Demo recording enabled -> {self._demo_frames_path}")
    ...
    def _process_frame(self, tick_data):
        ...
        rgb = self.rgb_front_transform(
            Image.fromarray(tick_data["rgb"])).unsqueeze(0).to(self.device).float()
        rgb_left = self.rgb_left_transform(
            Image.fromarray(tick_data["rgb_left"])).unsqueeze(0).to(self.device).float()
        rgb_right = self.rgb_right_transform(
            Image.fromarray(tick_data["rgb_right"])).unsqueeze(0).to(self.device).float()
        ...
        mes = torch.from_numpy(np.array(cmd_one_hot)).float().unsqueeze(0).to(self.device)
        ...
        return {
            ...
            "target_point": torch.from_numpy(
                tick_data["target_point"]).float().to(self.device).view(1, -1),
            "lidar": torch.from_numpy(
                tick_data["lidar"]).float().to(self.device).unsqueeze(0),
        }
    ...
    def run_step(self, input_data, timestamp):
        ...
        if self._demo_frames_path is not None:
            raw_bgr = input_data["rgb"][1][:, :, :3]
            raw_rgb = cv2.cvtColor(raw_bgr, cv2.COLOR_BGR2RGB)
            Image.fromarray(raw_rgb).save(
                self._demo_frames_path / ("frame_%06d.png" % self._demo_frame_idx)
            )
        ...
        window = self._build_temporal_window()
